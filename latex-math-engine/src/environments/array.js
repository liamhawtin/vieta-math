...
function parseArray(
    ...
    while (true) {  // eslint-disable-line no-constant-condition
        ...
        if (Array.isArray(cell)) {
            if (cell.length === 1 && cell[0].type === "ordgroup") {
                cell = cell[0];
            } else {
                // Create location for empty cells or cells that need wrapping
                let cellLoc = null;
                if (isEmpty) {
                    // For empty cells, create a zero-width location at the current position
                    const cellStartPos = parser.fetch().loc.start;
                    cellLoc = new SourceLocation(parser.gullet.lexer, cellStartPos, cellStartPos);
                }

                let loc = cellLoc;
                if (!loc && cell.length > 0) {
                    const first = cell[0];
                    const last = cell[cell.length - 1];
                    if (first?.loc && last?.loc) {
                        loc = new SourceLocation(first.loc.lexer, first.loc.start, last.loc.end);
                    }
                }
        ...
        if (next === "&") {
            ...
            // Look ahead to check whether \end is coming immediately
            const lookahead = parser.fetch().text;
            if (lookahead !== "\\end") {
                row = [];
                body.push(row);
                beginRow();
...
const mathmlBuilder: MathMLBuilder<"array"> = function(group, options) {
    ...
    for (let i = 0; i < group.body.length; i++) {
        ...
        for (let j = 0; j < rw.length; j++) {
            const cell = new mathMLTree.MathNode("mtd", [
                    mml.buildGroup(rw[j], options.havingStyle(Style.TEXT))
                ]);
            let defGapValue = (group.arraystretch === 0.5)
            ...
            defGapValue = defGapValue/2;

            let botRowGap;
            const gap = group.rowGaps[i];
            if (i === group.body.length - 1) {
                botRowGap = gap && gap.unit === "em" ? gap.number : 0;
            } else {
                botRowGap = defGapValue + (gap && gap.unit === "em" ? gap.number : 0);
            }

            // spacing
            let style = "";
            // only add top padding if not the first row
            if (i !== 0) {
                style += `padding-top: ${defGapValue}em;`;
            }
            style += `padding-bottom: ${botRowGap}em;`;

            if (j !== 0) {
                style += `padding-left: ${spacingValue/2}em;`;
            }
            if (j !== rw.length - 1) {
                style += `padding-right: ${spacingValue/2}em;`;
            }

            if (group.cols && group.cols[j] && group.cols[j].type === "align") {
                style += `text-align: -webkit-${alignMap[group.cols[j].align]};`;
            }
    ...
    if (group.colSeparationType !== "delimited") {
        const margin = `${spacingValue/2}em`;
        //
        //table.setStyle("marginLeft", margin);
        //table.setStyle("marginRight", margin);
    }

    // TODO: Currently NO support for columnlines !!!
    // To place a line on an edge we'll use <menclose>, if necessary.
    let menclose = "";
    if (group.cols && group.cols.length > 0) {
        if (group.cols[0].type === "separator") {
            menclose += "top ";
        }
        if (group.cols[group.cols.length - 1].type === "separator") {
            menclose += "bottom ";
        }
    }
    ...
    table.setAttribute("scriptlevel", "0");
...
const alignedHandler = function(context, args) {
    ...
    res.body.forEach(function(row) {
        for (let i = 1; i < row.length; i += 2) {
            ...
            const ordgroup = assertNodeType(row[i], "ordgroup");
...
defineEnvironment({
    ...
    handler(context, args) {
        ...
        return parseArray(context.parser, res);
    },
    htmlBuilder,
    mathmlBuilder,
});
...
defineEnvironment({
    ...
    handler(context) {
        ...
        const res: ParseNode<"array"> =
            parseArray(context.parser, payload);
        ...
        res.colSeparationType = "delimited";
...
defineEnvironment({
    ...
    handler(context) {
        ...
        const res = parseArray(context.parser, payload);
