...
defineFunctionBuilders({
    ...
    mathmlBuilder(group, options) {
        ...
        if (group.sub) {
            const subOptions = options.havingStyle(options.style.sub());
            children.push(mml.buildGroup(group.sub, subOptions));
        }

        if (group.sup) {
            const supOptions = options.havingStyle(options.style.sup());
            children.push(mml.buildGroup(group.sup, supOptions));
        }

        let nodeType: MathNodeType;

        if (group.placementMode === "limit") {
            if (!group.sub) {
                nodeType = "mover";
            } else if (!group.sup) {
                nodeType = "munder";
        ...
        } else if (group.placementMode === "script") {
            if (!group.sub) {
                nodeType = "msup";
            } else if (!group.sup) {
                nodeType = "msub";
        ...
        // Default auto-detection logic
            ...
            if (Number.isFinite(size) && size > 0) {
                nodeType = "msup";
            } else if (base && base.type === "op" && base.limits &&
        ...
        let baseMML = group.base ? mml.buildGroup(group.base, options) : null;

        if (["msup", "msub", "msubsup"].includes(nodeType) && group.base && group?.base?.type !== "op") {
            if (group.base.type === "ordgroup" && !group.base.body?.length) {
                children.unshift(baseMML);
                return new mathMLTree.MathNode(nodeType, children);
            }

            if (baseMML.type === "mo") {
                baseMML.setAttribute("rspace", "0em");
                baseMML.setAttribute("lspace", "0em");
            }

            baseMML = Array.isArray(baseMML) ? baseMML[0] : baseMML;
            let strippedBase = baseMML.clone();

            if (
                strippedBase.type === "mover" &&
                strippedBase.attributes["accent"] === "true" &&
                strippedBase.children.length > 0 &&
                strippedBase.children[0].type === "mrow"
            ) {
                // Replace strippedBase with its <mrow> child
                strippedBase = strippedBase.children[0];
            }

            strippedBase.clearAttributes(["data-range-start", "data-range-end"]);
            strippedBase.classes = ["ghost-base"];
            strippedBase.setAttribute("rspace", "0em");
            strippedBase.setAttribute("lspace", "0em");
            strippedBase.setStyle("width", "0");
            strippedBase.setStyle("visibility", "hidden");
            strippedBase.setStyle("marginLeft", "0");
            strippedBase.setStyle("padding", "0");

            const mpadded = new mathMLTree.MathNode("mpadded", [strippedBase]);
            mpadded.setStyle("width", "0");
            mpadded.classes = ["ghost-base"];
            children.unshift(mpadded);

            if (group.base && group.base.loc && group.loc) {
                group.loc.start = group.base.loc.end;
            }

            return [baseMML, new mathMLTree.MathNode(nodeType, children)];
        } else if (group.base) {
            children.unshift(baseMML);
        } else {
            const emptyGroup = new mathMLTree.MathNode("mrow");
            children.unshift(emptyGroup);
        }
