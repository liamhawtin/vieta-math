...
defineFunction({
    ...
    mathmlBuilder(group, options) {
        ...
        if (group.body) {
            // build upper (above arrow) label
            const upperNode = mml.buildGroup(group.body, options);
            upperNode.setStyle("padding-left", "0.3em");
            upperNode.setStyle("padding-right", "0.6em");

            if (group.below) {
                // build lower (below arrow) label
                const lowerNode = mml.buildGroup(group.below, options);
                lowerNode.setStyle("padding-left", "0.3em");
                lowerNode.setStyle("padding-right", "0.6em");

                node = new mathMLTree.MathNode(
                    "munderover", [arrowNode, lowerNode, upperNode]
                );
                node.classes.push("arrow");
            } else {
            ...
            const emptyNode = new mathMLTree.MathNode("mrow");
            node = new mathMLTree.MathNode("mover", [arrowNode, emptyNode]);
        }
