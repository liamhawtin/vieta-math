...
const mathmlBuilder: MathMLBuilder<"operatorname"> = (group, options) => {
    ...
    identifier.setAttribute("data-function", "true");

    identifier.setAttribute('data-range-start', group.loc.start.toString());
    identifier.setAttribute('data-range-end', group.loc.end.toString());

    if (group.limits) {
        identifier.setAttribute('limits', 'true');
    }

    if (group.parentIsSupSub) {
        return identifier;
    } else {
        return mathMLTree.newDocumentFragment([identifier]);
    }
};
...
defineFunction({
    ...
    handler: ({parser, funcName}, args) => {
        ...
        return {
            ...
            limits: (funcName === "\\operatornamewithlimits"),
