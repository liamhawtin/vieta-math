...
const mathmlBuilder = (group, options) => {
    const argStyle = adjustStyle(group.size, options.style);
    const node = new mathMLTree.MathNode("mfrac", [
        mml.buildGroup(group.numer, options.havingStyle(argStyle.fracNum())),
        mml.buildGroup(group.denom, options.havingStyle(argStyle.fracDen())),
    ]);

    if (group.wasInfix) {
        node.classes.push("infix");
    }
    ...
    // Style & scriptlevel control
    const style = adjustStyle(group.size, options.style);

    const sizeToScriptLevel = {
        0: 0, // display
        1: 0, // text
        2: 1, // script
        3: 2, // scriptscript
    ...
    if (group.forceScriptLevel) {
        // Keep the scriptlevel of the surrounding context
        const currentLevel = sizeToScriptLevel[options.style.size] ?? 0;
        node.setAttribute("scriptlevel", String(currentLevel));
        node.setAttribute("displaystyle", "true");
    } else if (style.size !== options.style.size) {
        // Normal case: adjusted style changes scriptlevel
        node.setAttribute(
            "scriptlevel",
            String(sizeToScriptLevel[style.size] ?? 0)
        );
        node.setAttribute("displaystyle", "true");
    }
    ...
    if (group.leftDelim != null || group.rightDelim != null) {
        ...
        const nodeWithFrac = mml.makeRow(withDelims);
        nodeWithFrac.classes.push("delimited-frac");
