...
const checkErrors = function(
    ...
    errors.clearErrors();
    try {
        generateParseTree(expression, {throwOnError: false});
    } catch {}
    return errors.getErrors();
};

const expandString = function(expression: string): string {
    const normalizer = new LatexNormalizer(expression);
    const expandedString = normalizer.normalize();
    return expandedString;
}

const exportString = function(expression: string): string {
    const exporter = new LatexExporter(expression);
    const exportedString = exporter.export();
...
export default {
    ...
    expandString,
    /**
     * Exports LaTeX from WYSIWYG editor to human-readable, copy-paste ready format
     */
    exportString,
    /**
     *
