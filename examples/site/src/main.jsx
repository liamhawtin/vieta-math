...
function App() { const page = document.body.dataset.page; let child = <Home />; if (page === "standalone") child = <><section className="page-intro"><p className="eyebrow">Standalone input</p><h1>A math field that belongs to your application.</h1><p>Try editing and exporting one expression, then inspect the optional shared tools a host can mount.</p><a className="page-reference" href="https://github.com/liamhawtin/vieta-math/blob/main/docs/standalone.md">Read all standalone options and methods <span aria-hidden="true">→</span></a></section><Standalone /></>; if (page === "theme") child = <><section className="page-intro"><p className="eyebrow">Theming contract</p><h1>Make the editor match a real host.</h1><p>VietaMath follows explicit host theme values—not a browser preference alone.</p></section><Standalone themed /></>; if (page === "prosemirror") child = <><section className="page-intro"><p className="eyebrow">ProseMirror integration</p><h1>Inline math that stays part of the document.</h1><p>Insert a node, edit inside it, then continue navigating the surrounding text.</p><a className="page-reference" href="https://github.com/liamhawtin/vieta-math/blob/main/docs/prosemirror.md">Read the ProseMirror guide <span aria-hidden="true">→</span></a></section><ProseMirrorDemo /></>; return <Shell>{child}</Shell>; }
